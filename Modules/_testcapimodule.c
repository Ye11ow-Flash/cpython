...
// static PyCodeObject *g_codeobject_callback = NULL;
// static PyCodeObject_EventCallback orig_callback = NULL;
static int code_object_created_counts, code_object_destroyed_counts;

static void
code_object_modified_callback(PyCodeObject_Event event, PyCodeObject *code_object)
{
    if(event == PYCODEOBJECT_EVENT_CREATED){
        code_object_created_counts++;
    } 
    else if (event == PYCODEOBJECT_EVENT_DESTROYED){
        code_object_destroyed_counts++;
    }
}

// static PyCodeObject *
// add_code_object_watcher(PyObject *self, PyObject *kind)
// {
//     int watcher_id;
...
set_code_object_event_callback() {
    code_object_created_counts = 0;
    code_object_destroyed_counts = 0;
    PyCodeObject_SetEventCallback(code_object_modified_callback);
    Py_RETURN_NONE;
}

static PyObject *
get_code_object_created_counts() {
    return PyLong_FromLong(code_object_created_counts);
}
...
static PyMethodDef TestMethods[] = {
    ...
    // Jaineel
    {"set_code_object_event_callback", set_code_object_event_callback, METH_NOARGS, NULL},
    {"get_code_object_created_counts", get_code_object_created_counts, METH_NOARGS, NULL},
